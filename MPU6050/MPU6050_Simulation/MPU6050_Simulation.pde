...
void setup()  { 
  ...
  port = new Serial(this, "COM24", 115200);
  port.clear();
  port.bufferUntil(lf);
} 
...
void draw()  { 
  ...
  textSize(15);
  String angles = "Pitch: " + (int) angle_x + " Roll: " + (int) angle_y + " Yaw: " + (int) angle_z;
  fill(255, 0, 0);
  text(angles, 20, 20); 
} 
...
void serialEvent(Serial p) {
  ...
  try {
    ...
    if (data[0].equals("#")) {
        angle_x = float(data[1]);
        angle_y = float(data[2]);
        angle_z = float(data[3]);    
    }
